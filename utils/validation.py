...
def validate_field_value(
    ...
    if len(cleaned) > max_length:
        return ValidationResult(
            is_valid=False,
            error_message=(
                f"❌ Слишком длдемо_кодое значение. "
    ...
    elif field_type == "demo_document_ref":
        cfg = _get_config()
        pattern = re.compile(cfg["demo_document_ref_PATTERN"])
