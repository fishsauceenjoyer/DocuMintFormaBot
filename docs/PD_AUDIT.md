...
## ✅ Результаты аудита локалей (задача 1.4)

Проверены `locales/ru.json`, `locales/en.json`, `locales/uk.json`:

- Ключей с «паспорт», «ИНН», «СНИЛС», «регистрация» — **не найдено**.
- Единственные совпадения — «адрес доставки» / "Delivery address" в ключах
  `delivery_prompt` и `delivery_format_error`. Это адрес **доставки посылки**
  (логистика: ФИО получателя, телефон, постамат), а не адрес регистрации/проживания
  из ПД-документов. Ключи сохранены, так как фича доставки остаётся в продукте.

## ✅ Результаты аудита клавиатур (задача 1.5)

Проверен `keyboards/buttons.py`:

- Хардкод callback_data с `passport`, `inn`, `address` — **не найдено**.
- Кнопки выбора документов (`doc_{code}`) генерируются **динамически** из
  `DOCUMENT_TEMPLATES` (функция `document_keyboard(docs)`). После удаления
  ПД-шаблонов (задача 1.2) кнопки visa/passport/criminal_record/apostille
  больше не создаются — остаются только постеры.
- Кнопка `delivery_yes`/`delivery_no` относится к логистике доставки и сохранена.
