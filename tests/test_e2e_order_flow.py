...
async def test_e2e_full_order_flow(clean_user_sessions):
    ...
    # ── Step 2: Choose document (demo_service) → entering_quantity ──────────
    from handlers.order import process_document_choice
    from handlers.order import get_user_session

    callback1 = MockCallback(data="doc_demo_service", user_id=user_id)
    ...
    session["current_doc_type"] = "demo_service"
    ...
    session = await get_user_session(user_id)
    session["total_price"] = 35  # 1 × demo_service(35€)
    ...
    session = await get_user_session(user_id)
    session["cart"] = [
        {"type": "demo_service", "quantity": 1, "items": [
            {"full_name": "John Doe"},
        ]}
    ]
