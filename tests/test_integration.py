...
async def test_integration_full_order_flow(
    ...
    # ── Step 2: Choose document (demo_check_check) → entering_quantity ──
    from handlers.order import process_document_choice

    callback_doc = MockCallback(data="doc_demo_check_check", user_id=user_id)
    ...
    # demo_check_check template has: full_name, birth_date, birth_place
    ...
    try:
        ...
        assert order.total_price == 25  # 1 × demo_check_check (25 EUR)
        ...
        assert len(docs) == 1
        assert docs[0]["type"] == "demo_check_check"
    ...
    assert "demo_check check" in caption
