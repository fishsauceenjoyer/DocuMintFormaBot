...
class TestConstants:
    ...
    def test_demo_document_ref_pattern_is_string(self):
        """Verify demo_document_ref_PATTERN is a non-empty regex string."""
        from data.business_config import demo_document_ref_PATTERN

        assert isinstance(demo_document_ref_PATTERN, str)
        assert len(demo_document_ref_PATTERN) > 0
        # Compile to verify it's a valid regex
        re.compile(demo_document_ref_PATTERN)
    ...
    def test_routing_keys(self):
        ...
        assert "demo_service" in ROUTING_KEYS
        assert "demo_document" in ROUTING_KEYS
...
class TestGetTemplate:
    ...
    def test_returns_dict_for_known_code(self):
        ...
        result = get_template("demo_service")
    ...
    def test_returned_template_has_required_keys(self):
        ...
        template = get_template("demo_service")
...
class TestGetAllTemplates:
    ...
    def test_contains_known_documents(self):
        ...
        assert "demo_service" in codes
        assert "demo_document" in codes
...
class TestGetPricePln:
    ...
    def test_returns_positive_int_for_known_code(self):
        ...
        price = get_price_pln("demo_service")
    ...
    def test_specific_known_prices(self):
        ...
        assert get_price_pln("demo_service") == 150
        assert get_price_pln("demo_document") == 200
...
class TestGetPriceEur:
    ...
    def test_returns_positive_int_for_known_code(self):
        ...
        price = get_price_eur("demo_service")
        assert isinstance(price, int)
        assert price > 0
    ...
    def test_specific_known_prices(self):
        ...
        # These values come from config/templates.yaml
        assert get_price_eur("demo_service") == 35
        assert get_price_eur("demo_document") == 45
        assert get_price_eur("apostille") == 30


class Testdemo_documentPattern:
    """Tests for demo_document_ref_PATTERN validation."""

    def test_valid_demo_document_refs(self):
        """Verify valid demo_document numbers match the pattern."""
        from data.business_config import demo_document_ref_PATTERN

        pattern = re.compile(demo_document_ref_PATTERN)
        valid_numbers = [
            "AB1234567",
            "123456789",
            "ABC-123/45",
    ...
    def test_invalid_demo_document_refs(self):
        """Verify invalid demo_document numbers don't match the pattern."""
        from data.business_config import demo_document_ref_PATTERN
