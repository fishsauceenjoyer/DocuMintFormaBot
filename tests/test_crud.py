...
class TestDocumentTypeCRUD:
    ...
    def test_get_document_type_by_code(self, mock_db_session):  # noqa: F811
        ...
        doc_type = get_document_type(mock_db_session, "demo_service")
        assert doc_type is not None
        assert doc_type.code == "demo_service"
    ...
    def test_get_all_document_types(self, mock_db_session):  # noqa: F811
        ...
        assert len(types) >= 4  # demo_service, demo_document, demo_check_check, apostille
...
class TestOrderCRUD:
    ...
    def _create_test_order(self, db, order_id="ORDER_TEST001"):
        ...
        return create_order(
            ...
            documents=[{"type": "demo_service", "quantity": 1, "items": [{"full_name": "Test"}]}],
        )
    ...
    def test_create_order_item(self, mock_db_session):  # noqa: F811
        ...
        item = create_order_item(
            db=mock_db_session,
            order_id=order.id,
            document_type="demo_service",
            quantity=2,
            unit_price=150,
            data={"type": "demo_service", "items": [{"full_name": "Test"}]},
        )
        assert item.document_type == "demo_service"
