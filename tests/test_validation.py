...
class TestValidateFieldValue:
    ...
    def test_valid_demo_document_ref(self):
        result = validate_field_value("FB363261", "demo_document_ref", field_name="demo_document_ref")
        assert result.is_valid is True
        assert result.sanitized_value == "FB363261"

    def test_demo_document_ref_with_allowed_chars(self):
        result = validate_field_value("AB-123.45 / 678", "demo_document_ref", field_name="demo_document_ref")
        assert result.is_valid is True
        assert result.sanitized_value == "AB-123.45 / 678"

    def test_demo_document_ref_too_short(self):
        result = validate_field_value("AB", "demo_document_ref", field_name="demo_document_ref")
        assert result.is_valid is False
        assert "от 3" in result.error_message

    def test_demo_document_ref_with_invalid_chars(self):
        result = validate_field_value("AB@123", "demo_document_ref", field_name="demo_document_ref")
        assert result.is_valid is False

    def test_demo_document_ref_uppercased(self):
        result = validate_field_value("fb363261", "demo_document_ref", field_name="demo_document_ref")
        assert result.is_valid is True
        assert result.sanitized_value == "FB363261"
...
class TestFieldConstructor:
    ...
    def test_demo_document_ref_type_hint(self):
        f = Field("demo_document", "Number", "demo_document_ref")
        hint = f.type_hint()
...
class Testdemo_documentNumberEquivalenceClasses:
    """Equivalence classes and boundary values for ``demo_document_ref`` fields."""
    ...
    def test_valid_demo_document_accepted(self, value):
        result = validate_field_value(value, "demo_document_ref", field_name="demo_document")
        assert result.is_valid is True
    ...
    def test_invalid_demo_document_rejected(self, value, expected_error):
        result = validate_field_value(value, "demo_document_ref", field_name="demo_document")
        assert result.is_valid is False
        assert expected_error in result.error_message
...
class TestQuantityBoundaryValues:
    ...
    async def test_quantity_boundary(self, qty_data, should_accept, mock_fsm, clean_user_sessions):
        ...
        cb_doc = MockCallback(data="doc_demo_service")
